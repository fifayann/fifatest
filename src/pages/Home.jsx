    ...
    <div className="bg-fifa-light min-h-screen">
      ...
      <section className="max-w-[1400px] mx-auto px-4 py-6">
        <div className="relative rounded-2xl overflow-hidden bg-fifa-black text-white min-h-[520px] flex items-end">
          ...
          <div className="relative z-10 p-8 md:p-12 max-w-2xl">
            ...
            <h1 className="text-4xl md:text-6xl font-black leading-tight mb-4">{t.hero.title}</h1>
            <p className="text-gray-200 mb-8 text-lg max-w-md">{t.hero.desc}</p>
            <div className="flex flex-wrap gap-3">
              <Link to="/tickets" className="bg-fifa-bright text-white font-bold px-7 py-3.5 rounded-full hover:bg-blue-700 transition-colors flex items-center gap-2">
                <Ticket size={18} /> {t.hero.getTickets}
              </Link>
      ...
      <section className="max-w-[1400px] mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-3xl font-black text-black">{t.sections.starPlayers}</h2>
            <p className="text-gray-500 text-sm mt-1">{t.sections.starPlayersSub}</p>
          </div>
          <div className="flex gap-2">
            <button onClick={() => scrollPlayers(-1)} className="w-10 h-10 rounded-full bg-white border border-gray-300 flex items-center justify-center hover:bg-gray-100 transition-colors"><ChevronLeft size={20} /></button>
            <button onClick={() => scrollPlayers(1)} className="w-10 h-10 rounded-full bg-white border border-gray-300 flex items-center justify-center hover:bg-gray-100 transition-colors"><ChevronRight size={20} /></button>
          </div>
        </div>
      ...
      <section className="max-w-[1400px] mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-3xl font-black">{t.sections.featuredMatches}</h2>
          <div className="flex gap-2">
            <button onClick={() => scrollPosters(-1)} className="w-10 h-10 rounded-full bg-white border border-gray-300 flex items-center justify-center hover:bg-gray-100 transition-colors"><ChevronLeft size={20} /></button>
            <button onClick={() => scrollPosters(1)} className="w-10 h-10 rounded-full bg-white border border-gray-300 flex items-center justify-center hover:bg-gray-100 transition-colors"><ChevronRight size={20} /></button>
          </div>
        </div>
            ...
            <div key={i} className="shrink-0 w-[340px] relative rounded-2xl overflow-hidden group cursor-pointer shadow-md hover:shadow-2xl transition-all">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6">
                ...
                <Link to="/tickets" className="inline-flex items-center gap-2 mt-3 bg-white text-fifa-black text-sm font-bold px-5 py-2.5 rounded-full hover:bg-fifa-bright hover:text-white transition-colors">
                  <Ticket size={14} /> {t.sections.buyTickets}
                </Link>
              </div>
            </div>
          ))}
      ...
      {/* ===== FOLLOW THE ACTION ===== */}
      <section className="max-w-[1400px] mx-auto px-4 py-8">
        <h2 className="text-3xl font-black mb-6">{t.sections.followAction}</h2>
          ...
          {upcomingMatches.map((m) => (
            <div key={m.id} className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-xl transition-shadow">
              ...
              <Link to="/tickets" className="mt-4 pt-3 border-t border-gray-100 flex items-center justify-center gap-2 text-fifa-bright font-semibold text-sm hover:gap-3 transition-all">
                <Ticket size={14} /> {t.sections.buyTicketsLower}
              </Link>
            </div>
          ))}
        </div>
      ...
      <section className="max-w-[1400px] mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-3xl font-black">{t.sections.highlights}</h2>
          <button className="flex items-center gap-2 text-sm font-semibold hover:text-fifa-blue">{t.sections.seeAll} <ArrowRight size={16} /></button>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {highlights.map((h) => (
            <div key={h.id}
              ...
              <video src={h.video} muted loop playsInline preload="metadata" className="w-full h-full object-cover" />
              ...
              <span className="absolute bottom-3 left-3 bg-fifa-black text-white text-xs font-semibold px-3 py-1.5 rounded pointer-events-none">{h.title}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="tickets" className="max-w-[1400px] mx-auto px-4 py-8">
        <div className="relative rounded-2xl overflow-hidden bg-fifa-black p-8 md:p-12">
          <div className="text-center text-white mb-10">
            <h2 className="text-3xl md:text-4xl font-black mb-2">{t.ticketsCta.title}</h2>
            <p className="text-gray-300">{t.ticketsCta.dates}</p>
          </div>
          <div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {[
              { icon: Ticket, title: t.ticketsCta.lastMinute, desc: t.ticketsCta.lastMinuteDesc, cta: t.ticketsCta.buyNow },
              { icon: RefreshCw, title: t.ticketsCta.marketplace, desc: t.ticketsCta.marketplaceDesc, cta: t.ticketsCta.resell },
              { icon: Diamond, title: t.ticketsCta.hospitality, desc: t.ticketsCta.hospitalityDesc, cta: t.ticketsCta.buyPackages },
            ].map((card, i) => (
              <div key={i} className="bg-fifa-bright rounded-xl overflow-hidden">
                ...
                <div className="bg-white p-6">
                  ...
                  <Link to="/tickets" className="inline-block bg-fifa-bright text-white text-sm font-semibold px-5 py-2.5 rounded-full hover:bg-blue-700 transition-colors">{card.cta}</Link>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="max-w-[1400px] mx-auto px-4 py-8">
        <div className="bg-fifa-bright rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="text-white">
            <h3 className="text-2xl font-bold mb-2">{t.fifaPass.title}</h3>
            <p className="text-sm text-blue-100 max-w-2xl">{t.fifaPass.desc}</p>
          </div>
          <button className="flex items-center gap-2 bg-transparent border-2 border-white text-white font-semibold px-6 py-3 rounded-full hover:bg-white hover:text-fifa-bright transition-colors whitespace-nowrap">
            {t.fifaPass.moreInfo} <ArrowRight size={18} />
          </button>
        </div>
      </section>
