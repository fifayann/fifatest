...
export default function Tickets() {
  ...
  const handleSubmit = async () => {
    ...
    const { error } = await supabase.from('ticket_requests').insert([{
      ...
      match_id: 0, match_label: 'General ticket request', category: 'N/A', quantity: 1, total_price: 0,
    }]);
    setSubmitting(false);
    if (error) { setFormError(t.tickets.errorGeneric); console.error(error); }
  ...
  const handleValidate = async () => {
    ...
    setChecking(true);
    setResult(null);
    const { data: codeRow, error } = await supabase.from('ticket_codes').select('*').eq('code', code.trim()).single();
    if (error || !codeRow) { setResult('invalid'); setChecking(false); return; }
    if (codeRow.used) { setResult('used'); setChecking(false); return; }
    const { error: updateError } = await supabase.from('ticket_codes')
    ...
    <div className="bg-fifa-light min-h-screen">
      <div className="max-w-md mx-auto px-4 py-12">
        <Link to="/" className="inline-flex items-center gap-2 text-sm font-semibold text-gray-600 hover:text-fifa-blue mb-6">
          <ChevronLeft size={18} /> {t.tickets.back}
        </Link>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-8 shadow-lg">
          ...
          {step === 'form' && (
            <>
              <h1 className="text-2xl font-black text-center mb-2">{t.tickets.requestTitle}</h1>
              <p className="text-gray-500 text-sm text-center mb-6">{t.tickets.requestDesc}</p>
              <div className="space-y-3 mb-5">
                <input type="text" placeholder={t.tickets.fullName} value={form.fullName}
                ...
                <input type="email" placeholder={t.tickets.email} value={form.email}
                  onChange={(e) => setForm({ ...form, email: e.target.value })}
                  className="w-full px-4 py-3 rounded-lg border border-gray-300 text-sm focus:border-fifa-bright focus:outline-none" />
                <input type="tel" placeholder={t.tickets.phone} value={form.phone}
              ...
              <button onClick={handleSubmit} disabled={!canSubmit || submitting}
                ...
                {submitting ? <><Loader2 size={18} className="animate-spin" /> {t.tickets.submitting}</> : t.tickets.submit}
              </button>
            </>
          )}
          ...
          {step === 'validate' && (
            <>
              <h1 className="text-2xl font-black text-center mb-2">{t.tickets.validateTitle}</h1>
              <p className="text-gray-500 text-sm text-center mb-6">{t.tickets.validateDesc}</p>
              ...
              {result === 'valid' && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-5 text-center">
                  ...
                  <h3 className="font-bold text-green-700">{t.tickets.codeValid}</h3>
                  <p className="text-sm text-green-600 mt-1">{t.tickets.codeValidDesc}</p>
                </div>
              )}
              {result === 'used' && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-5 text-center">
                  ...
                  <h3 className="font-bold text-red-700">{t.tickets.codeUsed}</h3>
                  <p className="text-sm text-red-600 mt-1">{t.tickets.codeUsedDesc}</p>
                </div>
              )}
              {result === 'invalid' && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-5 text-center">
                  <XCircle size={36} className="text-red-500 mx-auto mb-2" />
                  <h3 className="font-bold text-red-700">{t.tickets.codeInvalid}</h3>
                  <p className="text-sm text-red-600 mt-1">{t.tickets.codeInvalidDesc}</p>
                </div>
              )}

              {result !== 'valid' && (
                <>
                  <input type="text" placeholder={t.tickets.codePlaceholder} value={code}
                  ...
                  <button onClick={handleValidate} disabled={!code.trim() || checking}
                    ...
                    {checking ? <><Loader2 size={18} className="animate-spin" /> {t.tickets.checking}</> : t.tickets.validate}
                  </button>
                </>
              )}
